...
class AutoswaggerIntegration:
    ...
    def __init__(
        self, rate_limit: int = 30, timeout: int = 10, max_concurrent: int = 10
    ):
    ...
    async def __aenter__(self):
        ...
        if custom_cert and os.path.exists(custom_cert):
            ...
            if os.getenv("FLASK_ENV") == "development":
                ...
                logger.warning(
                    "⚠️ SSL verification disabled for development environment"
                )
    ...
    async def discover_swagger_for_urls(
        self, urls: List[str]
    ) -> List[SwaggerDiscoveryResult]:
    ...
    async def _discover_swagger_for_host(
        self, host: str
    ) -> Optional[SwaggerDiscoveryResult]:
                    ...
                    if response.status == 200:
                        ...
                        if "json" in content_type or path.endswith(".json"):
                            try:
                                ...
                                result = await self._parse_swagger_data(
                                    host, swagger_url, swagger_data
                                )
                                if result:
                                    logger.info(
                                        f"📋 Found Swagger documentation at {swagger_url}"
                                    )
                                    return result
        ...
        try:
            ...
            for script in scripts:
                if script.string:
                    ...
                    if (
                        "swagger" in script.string.lower()
                        or "openapi" in script.string.lower()
                    ):
                        # Extract JSON data if embedded
                        json_match = re.search(
                            r'({.*?"swagger".*?})', script.string, re.DOTALL
                        )
                        if json_match:
                            try:
    ...
    async def _parse_swagger_data(
        self, host: str, swagger_url: str, data: Dict[str, Any]
    ) -> Optional[SwaggerDiscoveryResult]:
    ...
    async def _analyze_swagger_content(
        self, result: SwaggerDiscoveryResult, swagger_data: Dict[str, Any]
    ):
    ...
    def _detect_pii(self, content: str, result: SwaggerDiscoveryResult):
        ...
        for pii_type, pattern in pii_patterns.items():
            ...
            for match in matches:
                ...
                if not any(
                    example in pii_value.lower()
                    for example in ["example", "test", "sample", "demo"]
                ):
...
def get_autoswagger_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "include_non_get": os.getenv("AUTOSWAGGER_INCLUDE_NON_GET", "false").lower()
        == "true",
    }
    ...
    async with AutoswaggerIntegration(
        ...
        for result in results:
            swagger_data = {
                ...
                "security_issues": len(result.pii_detected)
