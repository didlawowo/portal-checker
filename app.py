...
def load_excluded_urls():
    ...
    if os.path.exists(EXCLUDED_URLS_FILE):
        try:
            with open(EXCLUDED_URLS_FILE, "r", encoding="utf-8") as file:
                ...
                if isinstance(data, list):
                    ...
                    logger.info(
                        f"✅ {len(excluded_urls)} URLs exclues chargées depuis YAML"
                    )
                    return excluded_urls
                else:
                    logger.error(
                        "❌ Format YAML incorrect pour les exclusions (doit être une liste)"
                    )
                    exit(1)
        except Exception as e:
        ...
        logger.info(
            "ℹ️ Aucun fichier d'exclusion trouvé, toutes les URLs seront testées"
        )
        return excluded_urls
